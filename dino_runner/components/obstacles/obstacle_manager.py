...
class ObstacleManager:
  def __init__(self):
    ...
    for i in range(2):
      ...
      self.set_pos_sprites[i].pos_init(self.pos_x)
    ...
    for i in range(2):
      ...
      self.pos_x += int((SCREEN_WIDTH/2))
    self.pos_x = SCREEN_WIDTH + int((SCREEN_WIDTH/2))
    for i in range(2):
  ...
  def update(self,game_speed,player):
    ####  Guia  ####
    for i in range(len(self.set_pos_sprites)):
      self.set_pos_sprites[i].rect.y = 450
      if self.set_pos_sprites[i].rect.x < -self.set_pos_sprites[i].rect.width:
